...
track_courses = {
    "항공드론 초급": {
        "required": [
            ("항공우주학개론", 2, ["25-1", "25-2"]), ("항공우주산업개론", 2, ["25-1", "25-2"]), ("드론테크노비즈니스개론", 3, ["25-2"]), ("혁신융합세미나(항공드론)", 1, ["25-1"]),
            ("항공드론창의설계", 3, ["25-2"]), ("비행원리및모의조종실습", 1, ["25-2"])
        ],
        "or_groups": []
    },
    "항공드론 시스템 심화": {
        "required": [
            ("전산응용제도", 3, ["25-1"]), ("항공우주구조역학", 3, ["25-1"]), ("항공역학", 3, ["25-1"]),
            ("계측공학", 3, ["25-1"]), ("배터리소재의이해", 3, ["25-2"]), ("열역학1", 3, ["25-1"])
        ],
        "or_groups": [
    ...
    "항공드론 AI 심화": {
        "required": [
            ("이산수학", 3, ["25-1"]), ("AI프로그래밍", 3, ["25-1"]), ("딥러닝", 3, ["25-1"]), 
            ("머신러닝입문", 3, ["25-1"]), ("드론강화학습", 3), ("지능센서공학", 3)
        ],
        "or_groups": [
...
def calculate_earned_credits(track_info, completed_courses):
    ...
    for group in track_info.get("or_groups", []):
        ...
        if not group_satisfied:
            available = [c for c in group if c[2] == TARGET_SEMESTER]
            if available:
